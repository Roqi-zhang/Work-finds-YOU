...
## 三、修复：投递管理里点「岗位画像 / 个人画像」看不到内容

现象：从投递管理进入 `/snapshot` 后显示空白，或提示「这个岗位下还没有生成个人画像」。

已看到的代码事实（`src/pages/Snapshot.tsx`、`src/pages/Delivery.tsx:297-309`）：
- 快照页要求 `?job=` 是 UUID，否则直接报「还没有可查看的分析快照」；投递记录传的是 `a.id`，本地手动新增或早期记录的 id 未必是岗位画像 UUID。
- 个人画像分支只查 `user_profiles.target_job_profile_id = jobId`；当画像是通用画像（`target_job_profile_id` 为空）或由另一条路径生成时，就查不到 → 报「还没有生成个人画像」。
- 访客期生成的画像 `user_id` 为空，客户端 RLS 读不到（与第一部分改动相关）。

具体是哪一条触发，实施第一步先用真实记录核对，再按下面修。

修复方案：
1. 投递记录补全岗位主键：新增/写入投递时一并存 `jobProfileId`（`src/lib/wfy.ts` 的记录模型），跳转用它而不是 `a.id`；老记录做一次本地迁移（按 job 映射补齐），补不上的隐藏这两个按钮而不是跳到空页。
2. 个人画像查找改为按「这个岗位当时用的那份画像」定位：先查该岗位最新一条 `match_reports` 拿 `user_profile_id`，直接按 id 读 `user_profiles`；没有匹配报告时再回退到 `target_job_profile_id = jobId`，仍没有则回退到该用户 `is_current = true` 的通用画像。
3. 访客数据可读：访客行改由 Edge Function 按 `guest_key` 代读（与第一部分同一套归属逻辑），保证不登录时快照也能看。
4. 空态文案区分「这条记录没有关联岗位画像」与「读取失败」，并给出「去工作台重新分析」的入口。
5. 验证：投递页任取三条记录（含手动新增的一条）分别点两个按钮，都能看到与当时分析一致的 key points + 8 维内容。
